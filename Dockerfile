...
COPY . .
RUN export MIX_ENV=dev && rm -Rf _build && cd apps/portal_int && mix setup && mix release
...
#USER "${USER}"
#COPY --from=build --chown="${USER}":"${USER}" /_build/dev/rel/portal_int ./
WORKDIR "/home/app"
COPY --from=build /_build/dev/rel/portal_int ./
ENTRYPOINT ["bin/portal_int"]
CMD ["start"]
